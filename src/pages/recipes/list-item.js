...
const RecipesListItem = ({ setIsLoading, isLoading, recipe }) => {
	...
	return (
		<div
			className={`flex flex-col sm:flex-row ${
				isImageLoading ? "" : "border border-[var(--foreground)]"
			} rounded mb-5`}
		>
			...
			<div className="flex-grow flex flex-col m-4">
				<p
					...
					} flex-1 text-left xs:text-sm sm:text-base max-h-[6rem] overflow-hidden font-bold text-[var(--foreground)] font-geist`}
				>
					{recipe.name}
				</p>
				<div
					className={`${
						isImageLoading ? "opacity-0" : "opacity-100"
					} flex-2 flex-col justify-evenly overflow-hidden`}
				>
